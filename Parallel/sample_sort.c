...
/*
  ...
  This is sequential.
  We can add the insertion sort fallback for whenever n < some_threshold for cache efficiency
*/
static void
      ...
      for (left = begin; left < end; left += 2 * width)
        {
          ...
          // if (left - right <= 32){ // cache friendly
          // insertion_sort(data, scratch, left, right);
          // merge_runs (....);
          // skip to next chunk();
          // }
...
static void
merge_sort_omp_rec ( sort_key_t *data,
                     sort_key_t *scratch,
                     size_t      begin,
                     size_t      end,
                     size_t      cutoff )
{
  ...
  // base case: fall back to serial sort for small sub-arrays
  // the overhead from omp is greater than the gain
  // WE CAN TEST THE CUTOFF
  if (end - begin <= cutoff) {
  ...
  // why tasks : simple. the load work could be non uniform.
  // tasks enable dynamic and asynchronous execution

  // shared(data, scratch) : we do not want to waste time and space
  // creating copy of the data and the scratch
  // (we know that there will be no collisions)
  ...
  // before mergin we are gonna wait
  #pragma omp taskwait
...
void
merge_sort_omp ( sort_key_t *data, sort_key_t *scratch, size_t begin, size_t end )
{
  size_t cutoff = 1024; // this must be tested

  // this is a parallel region
  // call to function here will be indirectly parallel 
  #pragma omp parallel
  {
    // only one thread must call this once
    // otherwise this thing will be executed as many times
    // as many processes there are
    #pragma omp single
...
*/
static void
sample_sort        ( sort_key_t    *keys,          // input keys, modified by local sorts
